...
function jsonToZod(schema) {
    switch (schema.type) {
        ...
        case "array": return z.array(jsonToZod(schema.items));
...
try {
    ...
    const res = await fetch(`${baseUrl}/tools`, { headers: { Authorization: `Bearer ${apiKey}` } });
    ...
    server.tool("not_found", {}, async (_args, _ctx) => ({
...
for (const def of toolDefs) {
    ...
    server.tool(def.name, shape, async (args, _ctx) => {
        ...
        try {
            if (def.method === "get") {
                ...
                res = await fetch(`${baseUrl}${def.endpoint}`, {
                    method: "POST",
                    headers: {
...
server.resource("dummy", new ResourceTemplate("dummy://{dummy}", { list: undefined }), async (uri, { dummy }) => {
